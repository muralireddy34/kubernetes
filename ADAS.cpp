...
Mat YOLO::resize_image(Mat srcimg, int* newh, int* neww, int* top, int* left)
{
	...
	*neww = this->inpWidth;
	Mat dstimg;
	cout<<*newh<< " " <<*neww<<" "<<this->keep_ratio<<(srch != srcw)<<endl;
	...
	cout<<*newh<< " " <<*neww<<" "<<dstimg.rows<<dstimg.cols<<endl;
	return dstimg;
}
...
void YOLO::drawPred(int classId, float conf, int left, int top, int right, int bottom, Mat& frame)   // Draw the predicted bounding box
{
	...
	// cout<<"label:"<<this->classes[0]<<endl;
	label = "car:" + label;
...
Mat YOLO::detect(Mat& srcimg)
{
	...
	cout<<"here now norm"<<dstimg.cols<< dstimg.rows<<endl;
	imshow("dstimg", dstimg);
	waitKey(0);
	imshow("srcimg", srcimg);
	waitKey(0);
	Mat blob = blobFromImage(dstimg);
	std::cout<<"blobs:::::::"<<blob.size<<std::endl;
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        if (confidence >= this->objThreshold) {
            ...
            if (max_class_score > this->confThreshold) {
                ...
                float w = pdata[2];
                float h = pdata[3];
                int left = int((x - 0.5f * w) * ratiow);
                int top = int((y + 0.25f*h) );
	...
	for (size_t i = 0; i < indices.size(); ++i)
	{
		...
		if ((boxes[idx].width < 2) || (boxes[idx].height < 2)){
			continue;
		}
		// if ((boxes[idx].x + boxes[idx].width > srcimg.cols) && boxes[idx].width < srcimg.cols){
		// 	boxes[idx].x=srcimg.cols - boxes[idx].width ;
		// }
		// if ((boxes[idx].y + boxes[idx].height > srcimg.rows) && boxes[idx].height < srcimg.rows){
		// 	boxes[idx].y=srcimg.rows - boxes[idx].height ;
		// }
		// imwrite("out.jpg",outimg);
		cout<<boxes[idx]<<endl;
		// if ((boxes[idx].area() > 0) && (boxes[idx].x < srcimg.cols) && (boxes[idx].y < srcimg.rows)
		// && boxes[idx].x >= 0 && boxes[idx].y > 0)
		if ((boxes[idx].area() > 0) && (boxes[idx].x < srcimg.cols) && (boxes[idx].y < srcimg.rows)) {
			
			Rect box = boxes[idx];
			cout<<"draw "<<box<<" "<<outimg.cols<<outimg.rows<<endl;
			cout<<class_ids[idx]<<confidences[idx]<<endl;
...
int main()
{
	YOLO yolo_model("/home/new/ADAS/YOLOP/weights/2yolop-640-640.onnx", 0.3, 0.3, 0.3);
	string imgpath = "/home/new/ADAS/YOLOP/inference/images/adb4871d-4d063244.jpg";
